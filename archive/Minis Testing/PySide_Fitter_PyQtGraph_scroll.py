...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Add Elastic Offset parameter inputs
        self.elastic_offset_spinbox = QDoubleSpinBox()
        self.elastic_offset_spinbox.setRange(-100.0, 100.0)  # Example range
        self.elastic_offset_spinbox.setSingleStep(0.1)
        self.elastic_offset_spinbox.setDecimals(3)
        self.elastic_offset_spinbox.setValue(0.0)  # Default value

        self.fix_elastic_offset_cb = QCheckBox("Fix")

        # Row widget for Elastic Offset
        row_elastic_offset = QWidget()
        row_elastic_offset_layout = QHBoxLayout(row_elastic_offset)
        row_elastic_offset_layout.setContentsMargins(0, 0, 0, 0)
        row_elastic_offset_layout.addWidget(self.elastic_offset_spinbox)
        row_elastic_offset_layout.addWidget(self.fix_elastic_offset_cb)

        # Insert the row above the Gauss/Lorentz FWHM inputs
        form_layout.insertRow(0, "Elastic Offset:", row_elastic_offset)
        ...
        self.elastic_offset_spinbox.valueChanged.connect(self.update_previews)
    ...
    def connect_plot_events(self):
        ...
        # PyQtGraph uses signals for mouse events
        ...
        # Do NOT connect rangeChanged - it causes feedback loops with preview updates
    ...
    def on_mouse_click(self, event):
        ...
        try:
            ...
            # Right click no longer clears selection (spacebar now does)
            # Exclude mode: toggle nearest point
            if self.exclude_mode:
                self._toggle_nearest_point_exclusion_xy(x, y)
                return
            # If clicked near a phonon marker, select it and start dragging
            kind, obj = self._nearest_target_xy(x, y)
            if kind is not None:
                self.set_selected(kind, obj)
                if kind == 'phonon' and obj is not None and mouse_evt.button() == LeftButton:
                    self._dragging = True
                    self._drag_artist = obj
                else:
                    self._dragging = False
                    self._drag_artist = None
            else:
                # Nothing nearby: start panning handled by ViewBox default
                self._dragging = False
                self._drag_artist = None
        except Exception:
            pass
    ...
    def on_mouse_move(self, event):
        ...
        try:
            if not self._dragging or self._drag_artist is None:
                return
            # pyqtgraph.scene().sigMouseMoved emits a QPointF; sigMouseClicked passes a MouseClickEvent
            try:
            ...
            mousePoint = vb.mapSceneToView(pos)
            x = float(mousePoint.x())
            # Update center of active artist only
            self._drag_artist['center'] = x
            if 'widgets' in self._drag_artist and 'center_spin' in self._drag_artist['widgets']:
                w = self._drag_artist['widgets']['center_spin']
                try:
            ...
            try:
                if 'area' in self._drag_artist:
                    del self._drag_artist['area']
            except Exception:
                pass
        ...
        if do_range and len(x_inc) > 0:
            try:
                ...
                if vb is not None:
                    ...
                    vb.setXRange(xmin - 0.05*xr, xmax + 0.05*xr, padding=0)
                    vb.setYRange(ymin - 0.05*yr, ymax + 0.08*yr, padding=0)
            ...
            for info in self.draggable_artists:
                ...
                if 'area' in info and info['area'] is not None:
                    try:
                        ...
                        a = dho_area_for_height_ui(h, e, g, l, dmp, Tval, x)
                else:
                    a = dho_area_for_height_ui(h, e, g, l, dmp, Tval, x)
